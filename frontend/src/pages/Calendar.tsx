...
function Calendar() {
  ...
  const [isGoogleCalendarReady] = useState(true);
  ...
  const handleAddEvent = (e: React.FormEvent) => {
    ...
    addEvent({ ...eventData, date: format(eventData.date, 'yyyy-MM-dd') });
                  ...
                  <div className="flex space-x-2">
                    {isGoogleCalendarReady && (
                      <GoogleCalendarButton
                        event={{
                          title: event.title,
                          startDate: format(event.date, 'yyyy-MM-dd'),
                          startTime: event.time,
                          endDate: format(event.date, 'yyyy-MM-dd'),
                          endTime: event.time,
                          // type: event.type, // Removed as it is not part of the GoogleCalendarButtonProps interface
                        }}
                        // onSuccess={handleGoogleCalendarSuccess} // Removed as it is not part of the GoogleCalendarButtonProps interface
